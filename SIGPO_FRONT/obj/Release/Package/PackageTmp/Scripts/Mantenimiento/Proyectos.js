...
$(function () {
    ...
    tabla1 = $('#tablaProyectos').DataTable({
        ...
        "aoColumns": [
            ...
            { "title": "Unidad Organizacional", "bSortable": false, bVisible: true, "data": "iCodUnidad" },
            {
...
let eventos = function () {
    $("#btnAgregarNuevo").click(function () {
        ...
        doTask('GET', "Maestros/listarUnidades/", obj, function (respuesta) {
            ...
            $.each(respuesta, function (key, value) {
                if (value.iAmbito == 1) {
                    $('#cmbUnidadOrga').append("<option value='" + value.iCodUnidad + "'data-value='" + JSON.stringify(value.iCodUnidad) + "'>" + value.vNombreUnidad + "</option>");
                }
            });
    ...
    $('#cmbUnidadOrga').on('change', function (e) {
        ...
        Codigo = parseInt(e.target.value, 10);

        // Mostrar el valor seleccionado en la consola (puedes utilizarlo según tus necesidades)
        console.log('Valor seleccionado:', Codigo);
    });
...
function pintarTabla(datos) {
    ...
    datos.forEach(dato => {
        if (dato.iAmbito === 2) {
            ...
            celdaUnidadZonal.textContent = dato.vNombreUnidad; // Suponiendo que 'vNombreUnidad' es una propiedad en tu objeto
